...
def getPieChart(df, columns, title='Graf', rotation = 45, sort_values = True, sort_ascending = True, limit = 100, precision = 0):
    '''Tato funkce akceptuje df a nazvy sloupcu, vraci HTML kod vertikalniho grafu'''
    cols = []
    for i, column in enumerate(columns):
        #raise error if not found
    ...
    total = df[columns[0]].sum()
    expl = [0.0 for _ in range(len(df.index))]
    #expl[0] = 0.15    
    az.pie(values,labels = key, explode = expl, shadow = True, autopct=lambda p: rd((p * total / 100),precision), counterclock=False, startangle=-270)
    plt.title(title)
    plt.show()
    buf = BytesIO()
    fig.savefig(buf, format="png", bbox_inches='tight')
